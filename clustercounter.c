...
void cluster_counter (int  no_of_things,  int *neighbors[], int * mask,
    ...
    int flag_ctr, this_thing, other_thing, repeat_ctr, coverage_ctr, i; 
    ...
    memset (clusters[0], 0, (no_of_res+1)*(no_of_res+1)*sizeof(int));
    cluster_count = 0;
    isolated_count = 0;
    for (this_thing=0; this_thing < no_of_res; this_thing++) {
	if (  subsequence [this_thing] ) {
	    if ( !flag_ptr[this_thing] ) {
		isolated_count++;
		clusters [0][0]++;
