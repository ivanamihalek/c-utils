...
int read_pdb ( char * pdbname, Protein * protein, int * chain_number,
    ...
    int atomctr, resctr, no_res[2], ctr, nonblank;
	...
	while(fgets(line, BUFFLEN, fptr)!=NULL){
		...
		if ( chain_ctr > 2) {
		    ...
		    fprintf ( stderr, "Line %d: More than 2 protein chains in pdb .\n", line_ctr);
		    return 1;
		}
	...
	for (i=0; i<chain_ctr; i++) {
	    sequence[i] = emalloc ( no_res[i]*sizeof (Residue));
	}
	/*return values: */
	for (i=0; i<chain_ctr; i++) {
	    if ( invert ) {
		(protein+i)->sequence = sequence[1-i];
		(protein+i)->length   = no_res[1-i];
	    } else {
	...
	* chain_number = chain_ctr;
	...
	for (i=0; i<2; i++) {
	    if ( invert ) {
		sequence[1-i] = (protein+i)->sequence;
		no_res[1-i]   = (protein+i)->length;
	    } else {
	...
	sequence[chain_ctr][resctr].atom[atomctr].backbone = 0;
	if ( nonblank == 1) {
	    sequence[chain_ctr][resctr].atom[atomctr].backbone =
		!(  strcmp ( tmp, "N") && strcmp ( tmp, "C") && strcmp ( tmp, "O")  );
	} else if (  nonblank == 2) {
	    sequence[chain_ctr][resctr].atom[atomctr].backbone = ! strcmp ( tmp, "CA" );
	}
	/* printf ( " %4d %4d %4s is backbone: %1d \n", resctr, atomctr, */
	/* sequence[chain_ctr][resctr].atom[atomctr].type, sequence[chain_ctr][resctr].atom[atomctr].backbone); */
	strncpy ( tmp, line+PDB_ATOM_X, PDB_ATOM_X_LEN);
	tmp[PDB_ATOM_X_LEN] = '\0';
