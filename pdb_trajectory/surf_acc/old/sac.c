...
int main ( int argc, char * argv[]) {
    ...
    if ( argc < 2 ) {
	fprintf (stderr, "Usage: %s <pdb file name> [-n] [-l <lig type 1> <lig type 2> ...] [-s <chain res 1>   <chain res 2> ...]\n", argv[0]);
	exit (1);
    }
...
# endif
   ...
   for (i=0; i< chain_number; i++ ) {
       sprintf ( filename, "water_acc.%d.table", i+1);
       //sprintf ( filename, "water_flow.%d.table", i+1);
	if ( ! (fptr = efopen ( filename, "w")) ) exit (1);
	
       sprintf ( filename, "water_ids.%d.table", i+1);
	...
	for ( resctr=0; resctr< (protein+i)->length; resctr++) {
	    ...
	    fprintf ( fptr,"  %5s %5d  %c  %8.3lf   %8.3lf  %8.3lf   %4d  ", (protein+i)->sequence[resctr].pdb_id, resctr+1,
	    	     (protein+i)->sequence[resctr].res_type_short, dist_to_water[i][resctr],  avg_nr_of_nbrs[i][resctr],
	    	     (double)adjusted_list_length/frame_ctr,  curr_list_length[i][resctr]);
