/* find transformation which takes one pdb into another if partial alignment is known*/
# include <stdio.h>
# include <stdlib.h>
# include <math.h>
# include <string.h>
...
int main ( int argc, char * argv[]) {
    ...
    if ( argc < 3 ) {
	printf ( "Usage: %s <pdbname1> <pdbname2> <alignment file>.\n", argv[0] );
...
int  calphas_to_XY ( double *** xptr,  double ***yptr, char * name_x, char *name_y, int * no_matching_residues){

    int s, pos;
    char * seq_x = NULL, * seq_y = NULL;
    int resctr1, resctr2, ctr;
    double **x, **y;
    int find_calpha ( Residue * sequence, int no_res, int resctr, int x_ctr, double ** x);
    
    /* sanity: find the names in the alignment */
    for ( s=0; s< alignment.number_of_seqs; s++) {
	if ( ! strcmp(alignment.name[s], name_x ) ) {
	    printf ("found %s\n", name_x);
	    seq_x = alignment.sequence[s];
	} else if ( ! strcmp(alignment.name[s], name_y ) ) {
	    printf ("found %s\n", name_y);
	    seq_y = alignment.sequence[s];
	}
    }
    if ( ! seq_x ) {
	fprintf ( stderr, "Error:  %s not found in the alignment.\n", name_x);
	exit (1);
    }
    if ( ! seq_y ) {
	fprintf ( stderr, "Error:  %s not found in the alignment.\n", name_y);
    ...
    ctr = 0;
    /* find size of the system */
    for ( pos=0; pos < alignment.length; pos ++ ) {
	if ( ( seq_x[pos] != '.')  && ( seq_y[pos] != '.') ) {
	    ctr++;
	}
    }
    *no_matching_residues = ctr; /* this is called no_vectors in the main */
    
    /* allocate */
    if ( ! (x = dmatrix (N+1, ctr)))  exit(1);
    if ( ! (y = dmatrix (N,   ctr)))  exit(1);
    ...
    for ( pos=0; pos < alignment.length; pos ++ ) {
	if (  seq_x[pos] != '.'  &&  seq_y[pos] != '.' ) {
	    if ( find_calpha ( sequence1, no_res_1, resctr1, ctr, x) ) {
		fprintf ( stderr, "Error constructing x for %s.\n", name_x);
		exit (1);
	    }
	    if (find_calpha ( sequence2, no_res_2, resctr2, ctr,  y) ) {
		fprintf ( stderr, "Error constructing y for %s.\n", name_y);
		exit (1);
	    }
	    ctr ++;
	}
  	if ( seq_x[pos] != '.' ) resctr1++;
	if ( seq_y[pos] != '.' ) resctr2++;
  }
