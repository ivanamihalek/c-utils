...
# define  MAX_RES 10000  /* count atoms in each resdiue, see read_pdb below */
...
int find_contactcs (double cutoff_dist) {
    ...
    printf  ("# %2s %3s     %3s  %3s  %8s  \n", "id", "aa", "noc", "bb", "min_dist");
	...
	if ( noc ) {
	    printf  ("%5s %3s   ", sequence1[resctr1].pdb_id,  sequence1[resctr1].res_type);
	    printf  ("  %3d  %3d %8.3lf \n", noc, noc_bb, min_dist);
	}
...
int read_pdb ( char * pdbname, Residue ** sequence_ptr, int * no_res_ptr) {
    ...
    /* count residues */
    if ( !( number_of_atoms = calloc (MAX_RES, sizeof(int)))) return 1;
	...
	if( ! strncmp(line,"ATOM", 4) ||  ! strncmp(line,"HETATM", 6)){
		...
		if ( resctr >= MAX_RES ) {
		    fprintf ( stderr, "Error in geom_epitope.c: too many residues: %d\n", resctr);
		    return 1;
		}
		if ( resctr ) number_of_atoms [resctr-1] = atomctr;
		atomctr = 0;
		resctr ++;
	    }
	    atomctr++;
    ...
    *no_res_ptr = no_res;
    ...
    /* allocate space */
    sequence = NULL;
    if ( ! (sequence = calloc ( no_res, sizeof (Residue))) ) return 1;
    
    for (resctr=0; resctr < no_res; resctr++) {
	sequence[resctr].no_atoms = number_of_atoms[resctr];
	if ( !  (sequence[resctr].atom = (Atom*) calloc (  number_of_atoms[resctr], sizeof(Atom)) )) return 1;
    }
